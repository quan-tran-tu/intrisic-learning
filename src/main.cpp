...
void randomize(Tensor2D& t) {
    ...
    for(int i=0; i<size; ++i) t.data().get()[i] = dist(gen);
}
...
int main() {
    ...
    results.push_back(run_benchmark("GEMM loop unrolling", dim, 
        loop_unrolling_gemm, gemm_load.first, gemm_load.second, C_ref, C_out, A, B));
